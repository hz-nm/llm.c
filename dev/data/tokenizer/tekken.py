...
class Tekkenizer(Tokenizer):
    ...
    def __init__(
        ...
        *,
        name: str = "tekkenizer",
        _path: Optional[str] = "dev/data/tokenizer/tekken_240718.json",
    ):
    ...
    def encode(self, s: str, bos: bool, eos: bool) -> List[int]:
        ...
        # ! Following is a change of a line based on the original implementation from Andrej Karpathy.
        tokens: List[int] = self._model.encode(s, allowed_special={'<|endoftext|>'})
    ...
    def _decode_all_bytes(self, tokens: List[int], special_token_policy: SpecialTokenPolicy) -> List[str]:
        # Lump special and non-special tokens together to minimize calls to decode
        decoded: List[str] = []
        for is_special, group in groupby(tokens, lambda t: t < self.num_special_tokens):
            if is_special:
                ...
                decoded.append(self._model.decode_bytes([t - self.num_special_tokens for t in group]))
        return decoded
    ...
    def decode_bytes(self, tokens: List[int]) -> str:
        return "".join(self._decode_all_bytes(tokens, special_token_policy=self._special_token_policy))
